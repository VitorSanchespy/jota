...
// Configurações de banco alternativas
const configs = {
    ...
    // Configuração para XAMPP/WAMP/MAMP
    xampp: {
        database: 'npjdatabase',
        username: 'root',
        password: '',
...
// Detectar qual configuração usar (pode ser alterada manualmente)
const configMode = process.env.DB_MODE || 'docker';
const dbConfig = configs[configMode];

console.log(`🔧 Usando configuração: ${configMode}`);
    ...
    },
    {
        ...
        const processos = [
            ...
            },
            {
                ...
                sistema: 'Físico', // ENUM válido (corrigido de 'FÃ­sico')
        ...
        if (error.name === 'SequelizeConnectionRefusedError') {
            ...
            console.error(`   - Configuração atual: ${configMode}`);
            console.error('   - Para Docker: docker-compose up -d sistema-npj-db-1');
            console.error('   - Para XAMPP: DB_MODE=xampp node backend/popularComRoot.js');
            console.error(`   - Configurações: host=${dbConfig.host}:${dbConfig.port}, user=${dbConfig.username}`);
        } else if (error.name === 'SequelizeAccessDeniedError') {
            console.error('\n🔐 Problema de autenticação:');
            console.error('   - Verifique usuário e senha do MySQL');
            console.error('   - Crie o banco de dados "npjdatabase"');
        } else {
